...
def plot_chart(chart_type, path_to_png, path_to_log_list, ylim = None):
    N = len(path_to_log_list)
    HSV_tuples = [(x*1.0/N, 0.5, 1) for x in range(N)]
    RGB_tuples = map(lambda x: colorsys.hsv_to_rgb(*x), HSV_tuples)

    for ind, path_to_log in enumerate(path_to_log_list):
...
##        use_marker = False
        ...
        if not use_marker:
            ...
            while not ok:
                try:
                    ...
                    plt.plot(data[0], data[1], label = label, color = RGB_tuples[ind],
    ...
    plt.legend(path_to_log_list, loc='upper center', bbox_to_anchor=(.5, -0.1))
